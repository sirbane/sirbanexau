"""
dashboard.py (CLOUD VERSION) ─ Scuro Live Trading Dashboard
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Reads from Supabase instead of local file.
...
try:
    ...
    st.error("⚠️ Install: pip install supabase")
    st.stop()
...
def load_data_from_supabase() -> dict | None:
    try:
        response = supabase.table(TABLE_NAME).select("data").eq("id", 1).execute()
...
def render():
    ...
    if not data:
        st.error(
            "⚠️  **No data in Supabase.** Make sure:\n"
            "1. `upload_to_supabase.py` is running (alongside `mt5_history.py`)\n"
            "2. Supabase table `live_trading_data` exists\n"
        )
        st.stop()
    ...
    st.markdown(
        ...
        f'<span class="live-dot"></span>Cloud-synced every 10s · '
        f'Last data: {refreshed} · Data source: Supabase live_trading_data'
        f'</div>',
        unsafe_allow_html=True,
    )
